...
def get_data(interval):
    ...
    if interval == None:
        curs.execute("SELECT * FROM temperature")
         ...
         curs.execute("SELECT * FROM temperature WHERE timestamp>datetime('now','-%s hours')" % interval)
#        curs.execute("SELECT * FROM temperature WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hours') AND timestamp<=datetime('2013-09-19 21:31:02')" % interval)
...
def show_stats(option):
    ...
    curs.execute("SELECT timestamp,max(temperature) FROM temperature WHERE timestamp>datetime('now','-%s hour') AND timestamp<=datetime('now')" % option)
    rowmax=curs.fetchone()
    rowstrmax="{0}&nbsp&nbsp&nbsp{1}C".format(str(rowmax[0]),str(rowmax[1]))

    curs.execute("SELECT timestamp,min(temperature) FROM temperature WHERE timestamp>datetime('now','-%s hour') AND timestamp<=datetime('now')" % option)
#    curs.execute("SELECT timestamp,min(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    curs.execute("SELECT avg(temperature) FROM temperature WHERE timestamp>datetime('now','-%s hour') AND timestamp<=datetime('now')" % option)
#    curs.execute("SELECT avg(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    rows=curs.execute("SELECT * FROM temperature WHERE timestamp>datetime('now','-1 hour') AND timestamp<=datetime('now')")
